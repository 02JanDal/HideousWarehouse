...
{
	if (!value.isString()) {
		throw Exception(what + " is not a string");
	}
...
{
	if (!value.isBool()) {
		throw Exception(what + " is not a bool");
	}
	return value.toBool();
}
...
{
	if (!value.isDouble()) {
		throw Exception(what + " is not a double");
	}
				 ...
				 const QString &what = "Value")
{
	...
	if (fmod(doubl, 1) != 0) {
		throw Exception(what + " is not an integer");
	}
	return doubl;
}
				 ...
				 const QString &what = "Value")
{
	...
	if (!datetime.isValid()) {
		throw Exception(what + " is not a ISO formatted date/time value");
	}
	return datetime;
}
				 ...
				 const QString &what = "Value")
{
	...
	if (!url.isValid()) {
		throw Exception(what + " is not a correctly formatted URL");
	}
	return url;
}
				 ...
				 const QString &what = "Value")
{
	...
	if (string != QString::fromLatin1(string.toLatin1())) {
		throw Exception(what + " is not encodable as Latin1");
	}
			   ...
			   const QString &what = "__placeholder__")
{
	...
	if (!parent.contains(key)) {
		throw Exception(localWhat + "s parent does not contain " + localWhat);
	}
