...
{
	File::openAndWrite(filename, doc.toJson());
}
...
QJsonDocument Json::ensureDocument(const QByteArray &data)
{
	...
	if (error.error != QJsonParseError::NoError) {
		throw Exception("Error parsing JSON: " + error.errorString());
	}
	return doc;
}
QJsonDocument Json::ensureDocument(const QString &filename)
{
	return Json::ensureDocument(File::openAndRead(filename));
}
QJsonObject Json::ensureObject(const QJsonDocument &doc, const QString &what)
{
	if (!doc.isObject()) {
		throw Exception(what + " is not an object");
	}
	return doc.object();
}
...
{
	if (!doc.isArray()) {
		throw Exception(what + " is not an array");
	}
	return doc.array();
}
							   ...
							   const QString &what)
{
	if (!parent.contains(key)) {
		throw Exception(what + "s parent does not contain " + what);
	}
